...
if ( !function_exists( 'themerex_template_team_2_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_team_2_theme_setup', 1 );
	function themerex_template_team_2_theme_setup() {
		themerex_add_template(array(
			'layout' => 'team-2',
			'template' => 'team-2',
			'mode'   => 'team',
			'title'  => esc_html__('Team /Style 2/', 'kidsplanet'),
...
// Template output
if ( !function_exists( 'themerex_template_team_2_output' ) ) {
	function themerex_template_team_2_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			...
			><div class="sc_team_item_avatar column-2_5">
					<?php themerex_show_layout($post_options['photo']); ?>
				...
				<div class="sc_team_item_description"><?php themerex_show_layout(themerex_strshort($post_data['post_excerpt'], isset($post_options['descr']) ? $post_options['descr'] : themerex_get_custom_option('post_excerpt_maxlength_masonry'))); ?></div>
				<?php themerex_show_layout($post_options['socials']); ?>
			</div></div>
		<?php
