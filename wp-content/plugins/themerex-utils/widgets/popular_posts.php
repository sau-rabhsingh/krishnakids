 ...
 */
add_action( 'widgets_init', 'themerex_widget_popular_posts_load' );

/**
 * Register our widget.
 */
function themerex_widget_popular_posts_load() {
	register_widget('themerex_widget_popular_posts');
}

/**
 * Most popular and commented Widget class.
 */
class themerex_widget_popular_posts extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array('classname' => 'widget_popular_posts', 'description' => __('The most popular and most commented blog posts (extended)', 'trx_utils'));

		/* Widget control settings. */
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_popular_posts');

		/* Create the widget. */
		parent::__construct( 'themerex_widget_popular_posts', __('ThemeREX - Most Popular & Commented Posts', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget($args, $instance) {
		...
		$show_counters = isset($instance['show_counters']) && $instance['show_counters'] > 0 ? themerex_get_theme_option('blog_counters') : '';
		...
		if (count($tabs) > 0) {
			...
			echo themerex_do_shortcode('[trx_tabs style="1"]'.join($tabs).'[/trx_tabs]');
	...
	function form($instance) {
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_popular')); ?>"><?php _e('Most popular tab title:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_commented')); ?>"><?php _e('Most commented tab title:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('title_liked')); ?>"><?php _e('Most liked tab title:', 'trx_utils'); ?></label>
		...
		<p>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_counters')); ?>_1"><?php _e('Show', 'trx_utils'); ?></label>
