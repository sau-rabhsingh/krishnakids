 ...
 */
add_action( 'widgets_init', 'themerex_widget_advert_load' );

/**
 * Register our widget.
 */
function themerex_widget_advert_load() {
	register_widget( 'themerex_widget_advert' );
}

/**
 * Twitter Widget class.
 */
class themerex_widget_advert extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_advert', 'description' => __('Advertisement block', 'trx_utils') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_advert' );

		/* Create the widget. */
		parent::__construct( 'themerex_widget_advert', __('ThemeREX - Advertisement block', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		<div class="widget_advert_inner">
			...
			if ($advert_code!='') {
				echo force_balance_tags(themerex_substitute_all($advert_code));
			}
			?>
		</div>
		<?php
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Advertisement block', 'trx_utils') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_image' )); ?>"><?php _e('Image source URL:<br />(leave empty if you paste advert code)', 'trx_utils'); ?></label>
            ...
            <?php
			themerex_show_layout(themerex_show_custom_field($this->get_field_id( 'advert_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'advert_image' )), null));
			if ($advert_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_link' )); ?>"><?php _e('Image link URL:<br />(leave empty if you paste advert code)', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'advert_code' )); ?>"><?php _e('or paste Advert Widget HTML Code:', 'trx_utils'); ?></label>
