...
if ( !function_exists( 'themerex_template_services_1_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_services_1_theme_setup', 1 );
	function themerex_template_services_1_theme_setup() {
		themerex_add_template(array(
			'layout' => 'services-1',
			'template' => 'services-1',
			'mode'   => 'services',
			'title'  => esc_html__('Services /Style 1/', 'kidsplanet'),
			'thumb_title'  => esc_html__('Medium image1 (crop)', 'kidsplanet'),
			'w'		 => 370,
			'h'		 => 290
		));
	}
...
// Template output
if ( !function_exists( 'themerex_template_services_1_output' ) ) {
	function themerex_template_services_1_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php themerex_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				if ($post_data['post_icon'] && $post_options['tag_type']=='icons') {
					$html = themerex_do_shortcode('[trx_icon icon="'.esc_attr($post_data['post_icon']).'" shape="round"]');
					if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
						?><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php themerex_show_layout($html); ?></a><?php
					} else
						themerex_show_layout($html);
				} else {
					?>
				...
				<div class="sc_services_item_content">
					...
					if ($show_title) {
						if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
							?><h4 class="sc_services_item_title"><a href="<?php echo esc_url($post_data['post_link']); ?>"><?php themerex_show_layout($post_data['post_title']); ?></a></h4><?php
						} else {
							?><h4 class="sc_services_item_title"><?php themerex_show_layout($post_data['post_title']); ?></h4><?php
						}
					}
					?>
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !themerex_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_button sc_button_style_filled"><?php themerex_show_layout($post_options['readmore']); ?></a><?php
							}
						}
						?>
					</div>
