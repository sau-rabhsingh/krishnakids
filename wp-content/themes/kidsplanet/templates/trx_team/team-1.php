...
if ( !function_exists( 'themerex_template_team_1_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_team_1_theme_setup', 1 );
	function themerex_template_team_1_theme_setup() {
		themerex_add_template(array(
			'layout' => 'team-1',
			'template' => 'team-1',
			'mode'   => 'team',
			'title'  => esc_html__('Team /Style 1/', 'kidsplanet'),
	...
	function themerex_template_team_1_output($post_options, $post_data) {
		...
		?>
			<div<?php themerex_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				<div class="sc_team_item_avatar"><?php themerex_show_layout($post_options['photo']); ?></div>
