...
if ( !function_exists( 'themerex_template_clients_1_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_clients_1_theme_setup', 1 );
	function themerex_template_clients_1_theme_setup() {
		themerex_add_template(array(
			'layout' => 'clients-1',
			'template' => 'clients-1',
			'mode'   => 'clients',
			'title'  => esc_html__('Clients /Style 1/', 'kidsplanet'),
			'thumb_title'  => esc_html__('Original image', 'kidsplanet'),
			'w'		 => null,
			'h_crop' => null,
			'h'      => null
		));
...
if ( !function_exists( 'themerex_template_clients_1_output' ) ) {
	function themerex_template_clients_1_output($post_options, $post_data) {
		...
		?>
			<div<?php themerex_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?> class="sc_clients_item sc_clients_item_<?php echo esc_attr($post_options['number']) . ($post_options['number'] % 2 == 1 ? ' odd' : ' even') . ($post_options['number'] == 1 ? ' first' : '').(!empty($post_options['tag_class']) ? ' '.esc_attr($post_options['tag_class']) : ''); ?>"<?php themerex_show_layout(($post_options['tag_css'] ? ' style="'.esc_attr($post_options['tag_css']).'"' : '') . (!themerex_param_is_off($post_options['tag_animation']) ? ' data-animation="'.esc_attr(themerex_get_animation_classes($post_options['tag_animation'])).'"' : ''));?>>
				<?php if ($post_options['client_image']) { ?>
				<div class="sc_client_image"><?php
					themerex_show_layout(($post_options['client_link'] ? '<a href="'.esc_url($post_options['client_link']).'">'	: '')
