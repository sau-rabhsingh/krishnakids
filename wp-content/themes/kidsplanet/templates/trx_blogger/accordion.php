...
if ( !function_exists( 'themerex_template_accordion_theme_setup' ) ) {
	...
	function themerex_template_accordion_theme_setup() {
		themerex_add_template(array(
			...
			'mode'   => 'blogger',
			'title'  => esc_html__('Blogger layout: Accordion (Style 1)', 'kidsplanet')
			));
		themerex_add_template(array(
			'layout' => 'accordion-2',
			'template' => 'accordion',
			...
			'mode'   => 'blogger',
			'title'  => esc_html__('Blogger layout: Accordion (Style 2)', 'kidsplanet')
			));
...
if (!function_exists('themerex_template_accordion_add_scripts')) {
	//Handler of add_action('themerex_action_blog_scripts', 'themerex_template_accordion_add_scripts');
		...
		<div class="post_item sc_blogger_item sc_accordion_item<?php themerex_show_layout($post_options['number'] == $post_options['posts_on_page'] && !themerex_param_is_on($post_options['loadmore']) ? ' sc_blogger_item_last' : ''); ?>">
			...
			<div class="post_content sc_accordion_content">
				...
				if (themerex_param_is_on($post_options['info'])) {
					?>
					<div class="post_info">
						<span class="post_info_item post_info_posted_by"><?php esc_html_e('Posted by', 'kidsplanet'); ?> <a href="<?php echo esc_url($post_data['post_author_url']); ?>" class="post_info_author"><?php echo esc_html($post_data['post_author']); ?></a></span>
				...
				if (empty($post_options['readmore'])) $post_options['readmore'] = esc_html__('READ MORE', 'kidsplanet');
