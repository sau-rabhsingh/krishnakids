 ...
 */
add_action( 'widgets_init', 'themerex_widget_qrcode_load' );

/**
 * Register our widget.
 */
function themerex_widget_qrcode_load() {
	register_widget('themerex_widget_qrcode');
}

/**
 * QRCode Widget class.
 */
class themerex_widget_qrcode extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array('classname' => 'widget_qrcode', 'description' => __('Generate QRCode with your personal data or with any text (links)', 'trx_utils'));

		/* Widget control settings. */
		$control_ops = array('width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_qrcode');

		/* Create the widget. */
		parent::__construct( 'themerex_widget_qrcode', __('ThemeREX - QRCode generator', 'trx_utils'), $widget_ops, $control_ops );
	...
	function widget($args, $instance) {
		...
		if ($show_personal) 
			$output .= '
					...
					' . ($show_what==1 
						? '<p class="user_name odd first"><span class="theme_text">' . __('Name:', 'trx_utils') . '</span> <span class="theme_info">' . ($ufname) . ' ' . ($ulname) . '<span></p>'
							. ($ucompany ? '<p class="user_company even"><span class="theme_text">' . __('Company:', 'trx_utils') . '</span> <span class="theme_info">' . ($ucompany) . '<span></p>' : '')
							. ($uphone ? '<p class="user_phone odd"><span class="theme_text">' . __('Phone:', 'trx_utils') . '</span> <span class="theme_info">' . ($uphone) . '<span></p>' : '')
							. ($uemail ? '<p class="user_email even"><span class="theme_text">' . __('E-mail:', 'trx_utils') . '</span> <a href="' . esc_url('mailto:'.($uemail)) . '">' . ($uemail) . '</a></p>' : '')
							. ($usite ? '<p class="user_site odd"><span class="theme_text">' . __('Site:', 'trx_utils') . '</span> <a href="' . esc_url($usite) . '" target="_blank">' . ($usite) . '</a></p>' : '')
						: $text)
						. '
					</div>
					';
	...
	function form($instance) {
		...
		wp_enqueue_style( 'widget-qrcode-style',  trx_utils_get_file_url('widgets/qrcode/qrcode-admin.css'), array(), null );
		wp_enqueue_script('jquery-ui-tabs', false, array('jquery','jquery-ui-core'), null, true);
		wp_enqueue_script('widget-qrcode-script', trx_utils_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array(), null, false);

		/* Set up some default widget settings. */
		$address = explode(',', themerex_get_theme_option('user_address'));
		$defaults = array(
			'title' => '', 
			'description' => __('QR Code Generator (for your vcard)', 'trx_utils'),
			'ulname' => '', 
			'ufname' => '', 
			'ucompany' => '', 
			'uaddr' => '', 
		...
		<div class="widget_qrcode">
        	<div class="qrcode_tabs">
                <ul class="tabs">
                    <li class="first"><a href="#tab_settings"><?php _e('Settings', 'trx_utils'); ?></a></li>
                    <li><a href="#tab_fields" onmousedown="themerex_qrcode_init()"><?php _e('Personal Data', 'trx_utils'); ?></a></li>
                    <li><a href="#tab_text" onmousedown="themerex_qrcode_init()"><?php _e('Any Text', 'trx_utils'); ?></a></li>
                </ul>
                <div id="tab_settings" class="tab_content tab_settings">
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('title')); ?>"><?php _e('Title:', 'trx_utils'); ?></label>
                        <input class="fld_title" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('title')); ?>" name="<?php echo esc_attr($this->get_field_name('title')); ?>" value="<?php echo esc_attr($instance['title']); ?>" style="width:100%;" />
                    </p>
                    <p>
                        <label><?php _e('Show as QR Code:', 'trx_utils'); ?></label><br />
                        <input class="fld_show_what" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="1" type="radio" <?php echo ($instance['show_what']==1 ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_1"> <?php _e('Personal VCard', 'trx_utils'); ?></label>
                        <input class="fld_show_what" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0" name="<?php echo esc_attr($this->get_field_name('show_what')); ?>" value="0" type="radio" <?php echo ($instance['show_what']==0 ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_what')); ?>_0"> <?php _e('Any text', 'trx_utils'); ?></label>
                    </p>
                    <p>
                        <input class="fld_show_personal" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('show_personal')); ?>" name="<?php echo esc_attr($this->get_field_name('show_personal')); ?>" value="1" type="checkbox" <?php echo ($instance['show_personal']==1 ? 'checked="checked"' : ''); ?> />
                        <label for="<?php echo esc_attr($this->get_field_id('show_personal')); ?>"><?php _e('Show data under QR Code:', 'trx_utils'); ?></label>
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('width')); ?>"><?php _e('Width:', 'trx_utils'); ?></label>
                        <input onmousedown="themerex_qrcode_init()" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('width')); ?>" name="<?php echo esc_attr($this->get_field_name('width')); ?>" value="<?php echo esc_attr($instance['width']); ?>" style="width:100%;" class="fld_width" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('color')); ?>"><?php _e('Color:', 'trx_utils'); ?></label>
                        <input onmousedown="themerex_qrcode_init()" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('color')); ?>" name="<?php echo esc_attr($this->get_field_name('color')); ?>" value="<?php echo esc_attr($instance['color']); ?>" style="width:100%; background-color:<?php echo esc_attr($instance['color']); ?>" class="iColorPicker fld_color" />
                    </p>
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('bg')); ?>"><?php _e('Bg color:', 'trx_utils'); ?></label>
                        <input onmousedown="themerex_qrcode_init()" onfocus="themerex_qrcode_init()" id="<?php echo esc_attr($this->get_field_id('bg')); ?>" name="<?php echo esc_attr($this->get_field_name('bg')); ?>" value="<?php echo esc_attr($instance['bg']); ?>" style="width:100%; background-color:<?php echo esc_attr($instance['bg']); ?>" class="iColorPicker fld_bg" />
                    </p>
                </div>
                <div id="tab_fields" class="tab_content tab_personal">
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ulname')); ?>"><?php _e('Last name:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ufname')); ?>"><?php _e('First name:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ucompany')); ?>"><?php _e('Company:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('uphone')); ?>"><?php _e('Phone:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('uaddr')); ?>"><?php _e('Address:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ucity')); ?>"><?php _e('City:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('upostcode')); ?>"><?php _e('Post code:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('ucountry')); ?>"><?php _e('Country:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('uemail')); ?>"><?php _e('E-mail:', 'trx_utils'); ?></label>
                    ...
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('usite')); ?>"><?php _e('Web Site URL:', 'trx_utils'); ?></label>
                ...
                <div id="tab_text" class="tab_content tab_text">
                    <p>
                        <label for="<?php echo esc_attr($this->get_field_id('fld_text')); ?>"><?php _e('Text to show as QR Code:', 'trx_utils'); ?></label>
            ...
            <p>
                <input class="fld_button_draw" id="<?php echo esc_attr($this->get_field_id('button_draw')); ?>" name="<?php echo esc_attr($this->get_field_name('button_draw')); ?>" value="<?php echo esc_attr(__('Update', 'trx_utils')); ?>" type="button" />
                ...
                <label for="<?php echo esc_attr($this->get_field_id('auto_draw')); ?>"> <?php _e('Auto', 'trx_utils'); ?></label>
            </p>
...
<?php if ($instance['show_personal']==1) { ?>
				...
				<?php if ($instance['show_what']==1) { ?>
                    <li class="user_name odd first"><?php echo  __('Name:', 'trx_utils') . ' ' . ($instance['ufname']) . ' ' . ($instance['ulname']); ?></li>
                    <?php 
						echo  ($instance['ucompany'] ? '<li class="user_company even">' . __('Company:', 'trx_utils') . ' ' . ($instance['ucompany']) . '</li>' : '')
							. ($instance['uphone'] ? '<li class="user_phone odd">' . __('Phone:', 'trx_utils') . ' ' . ($instance['uphone']) . '</li>' : '')
							. ($instance['uemail'] ? '<li class="user_email even">' . __('E-mail:', 'trx_utils') . ' ' . '<a href="' . esc_url('mailto:'.($instance['uemail'])) . '">' . ($instance['uemail']) . '</a></li>' : '')
							. ($instance['usite'] ? '<li class="user_site odd">' . __('Site:', 'trx_utils') . ' ' . '<a href="' . esc_url($instance['usite']) . '" target="_blank">' . ($instance['usite']) . '</a></li>' : '');
					?>
				<?php } else { ?>
...
<?php } ?>
        ...
        <script type="text/javascript">
            jQuery(document).ready(function(){
				themerex_qrcode_init();
				themerex_color_picker();
            });
			function themerex_qrcode_init() {
            ...
            function themerex_qrcode_update(widget) {
				themerex_qrcode_show(widget, {
			...
			function themerex_qrcode_show(widget, vc, opt) {
				if (opt.show_what==1) {
			...
			if (!window.themerex_set_color_picker) {
				function themerex_set_color_picker(id_picker) {
	...
	function load_scripts() {
		...
		wp_enqueue_script('widget-qrcode-script', 		trx_utils_get_file_url('widgets/qrcode/jquery.qrcode-0.6.0.min.js'), array('jquery'), null, true);
	}
}
?>