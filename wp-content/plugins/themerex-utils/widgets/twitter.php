 ...
 */
add_action( 'widgets_init', 'themerex_widget_twitter_load' );

/**
 * Register our widget.
 * 'Twitter_Widget' is the widget class used below.
 */
function themerex_widget_twitter_load() {
	register_widget( 'themerex_widget_twitter' );
}

/**
 * Twitter Widget class.
 */
class themerex_widget_twitter extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_twitter', 'description' => __('Last Twitter Updates. Version for new Twitter API 1.1', 'trx_utils') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_twitter' );

		/* Create the widget. */
		parent::__construct( 'themerex_widget_twitter', __('ThemeREX - Twitter', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		if (is_array($data) && count($data) > 0) {
			foreach ($data as $tweet) {
				...
				$output .= '<li class="theme_text' . ($cnt==$twitter_count-1 ? ' last' : '') . '"><a href="' . esc_url('https://twitter.com/'.($twitter_username)) . '" class="username" target="_blank">@' . ($tweet['user']['screen_name']) . '</a> ' . force_balance_tags(themerex_prepare_twitter_text($tweet)) . '</li>';
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'description' => __('Last Twitter Updates', 'trx_utils') );
		$instance = wp_parse_args( (array) $instance, $defaults ); 
		$title = isset($instance['title']) ? $instance['title'] : '';
		$twitter_username = isset($instance['twitter_username']) ? $instance['twitter_username'] : themerex_get_theme_option('twitter_username');
		$twitter_consumer_key = isset($instance['twitter_consumer_key']) ? $instance['twitter_consumer_key'] : themerex_get_theme_option('twitter_consumer_key');
		$twitter_consumer_secret = isset($instance['twitter_consumer_secret']) ? $instance['twitter_consumer_secret'] : themerex_get_theme_option('twitter_consumer_secret');
		$twitter_token_key = isset($instance['twitter_token_key']) ? $instance['twitter_token_key'] : themerex_get_theme_option('twitter_token_key');
		$twitter_token_secret = isset($instance['twitter_token_secret']) ? $instance['twitter_token_secret'] : themerex_get_theme_option('twitter_token_secret');
		$twitter_count = max(1, intval(isset($instance['twitter_count']) ? $instance['twitter_count'] : themerex_get_theme_option('twitter_count')));
		?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_count' )); ?>"><?php _e('Tweets count:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_username' )); ?>"><?php _e('Twitter Username:', 'trx_utils'); ?><br />(<?php _e('leave empty if you paste widget code', 'trx_utils'); ?>)</label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_consumer_key' )); ?>"><?php _e('Twitter Consumer Key:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_consumer_secret' )); ?>"><?php _e('Twitter Consumer Secret:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_token_key' )); ?>"><?php _e('Twitter Token Key:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'twitter_token_secret' )); ?>"><?php _e('Twitter Token Secret:', 'trx_utils'); ?></label>
