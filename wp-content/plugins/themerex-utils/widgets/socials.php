 ...
 */
add_action( 'widgets_init', 'themerex_widget_socials_load' );

/**
 * Register our widget.
 */
function themerex_widget_socials_load() {
	register_widget( 'themerex_widget_socials' );
}

/**
 * flickr Widget class.
 */
class themerex_widget_socials extends WP_Widget {
	...
	function __construct() {
		/* Widget settings. */
		$widget_ops = array( 'classname' => 'widget_socials', 'description' => __('Show site logo and social links', 'trx_utils') );

		/* Widget control settings. */
		$control_ops = array( 'width' => 200, 'height' => 250, 'id_base' => 'themerex_widget_socials' );

		/* Create the widget. */
		parent::__construct( 'themerex_widget_socials', __('ThemeREX - Show logo and social links', 'trx_utils'), $widget_ops, $control_ops );
	}
	...
	function widget( $args, $instance ) {
		...
		$text = isset($instance['text']) ? themerex_do_shortcode($instance['text']) : '';
		...
		<div class="widget_inner">
            <?php
				if ($show_logo) {
					...
					if ($logo_image=='')	$logo_image = $THEMEREX_GLOBALS['logo'];
					if ($logo_text=='')		$logo_text = $THEMEREX_GLOBALS['logo_text'];
					if ($logo_slogan=='')	$logo_slogan = $THEMEREX_GLOBALS['logo_slogan'];
				...
				if (!empty($text)) {
					?>
					<div class="logo_descr"><?php echo nl2br(themerex_do_shortcode($text)); ?></div>
                    <?php
				}
	...
	function form( $instance ) {
		...
		$defaults = array( 'title' => '', 'logo_image'=>'', 'logo_text'=>'', 'show_logo' => '1', 'show_icons' => '1', 'text'=>'', 'description' => __('Show logo and social icons', 'trx_utils') );
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'text' )); ?>"><?php _e('Description:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_image' )); ?>"><?php _e('Logo image:<br />(if empty - use logo from Theme Options)', 'trx_utils'); ?></label>
            ...
            <?php
			themerex_show_layout(themerex_show_custom_field($this->get_field_id( 'logo_media' ), array('type'=>'mediamanager', 'media_field_id'=>$this->get_field_id( 'logo_image' )), null));
			if ($logo_image) {
			?>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id( 'logo_text' )); ?>"><?php _e('Logo text:', 'trx_utils'); ?></label>
		...
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php _e('Show logo:', 'trx_utils'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_1"><?php _e('Show', 'trx_utils'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_logo')); ?>_0"><?php _e('Hide', 'trx_utils'); ?></label>
		</p>
		<p>
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php _e('Show social icons:', 'trx_utils'); ?></label><br />
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_1"><?php _e('Show', 'trx_utils'); ?></label>
			...
			<label for="<?php echo esc_attr($this->get_field_id('show_icons')); ?>_0"><?php _e('Hide', 'trx_utils'); ?></label>
		</p>
