...
if ( !function_exists( 'themerex_template_services_2_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_services_2_theme_setup', 1 );
	function themerex_template_services_2_theme_setup() {
		themerex_add_template(array(
			'layout' => 'services-2',
			'template' => 'services-2',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 2/', 'kidsplanet'),
...
// Template output
if ( !function_exists( 'themerex_template_services_2_output' ) ) {
	function themerex_template_services_2_output($post_options, $post_data) {
		$show_title = true;
					...
					<div class="sc_services_item_description">
						...
						if ($post_data['post_protected']) {
							...
							if (!empty($post_data['post_link']) && !themerex_param_is_off($post_options['readmore'])) {
								?><a href="<?php echo esc_url($post_data['post_link']); ?>" class="sc_services_item_readmore"><?php themerex_show_layout($post_options['readmore']); ?><span class="icon-right"></span></a><?php
							}
						}
						?>
					</div>
