...
if (!function_exists('themerex_post_theme_setup')) {
	...
	function themerex_post_theme_setup() {
		...
		if (isset($THEMEREX_GLOBALS['post_override_options']) && $THEMEREX_GLOBALS['post_override_options']['page']=='post') {
			add_action('themerex_action_post_before_show_override_options',		'themerex_post_before_show_override_options', 10, 2);
			add_action('themerex_action_post_after_show_override_options',		'themerex_post_after_show_override_options', 10, 2);
		...
		if (isset($THEMEREX_GLOBALS['post_override_options']) && $THEMEREX_GLOBALS['post_override_options']['page']=='post') {
			$THEMEREX_GLOBALS['post_override_options']['fields'] = array(
					...
					"info_reviews_1" => array(
						"title" => esc_html__('Reviews criterias for this post', 'kidsplanet'),
						"override" => "post",
						"divider" => false,
						"desc" => esc_html__('In this section you can put your reviews marks', 'kidsplanet'),
						"class" => "reviews_meta",
						"type" => "info"),
					"show_reviews" => array(
					...
					"reviews_marks" => array(
						"title" => esc_html__('Reviews marks',  'kidsplanet'),
						"override" => "post",
						"desc" => esc_html__("Marks for this review", 'kidsplanet'),
...
if (!function_exists('themerex_post_before_show_override_options')) {
	...
	function themerex_post_before_show_override_options($post_type, $post_id) {
        ...
        themerex_set_global_array('js_vars', 'reviews_max_level', (int) $max_level);
        themerex_set_global_array('js_vars', 'reviews_allow_user_marks', true);
        ?>
...
if ( !function_exists( 'themerex_post_list_post_types' ) ) {
	...
	function themerex_post_list_post_types($list) {
		$list['post'] = esc_html__('Post', 'kidsplanet');
		$list['page'] = esc_html__('Page', 'kidsplanet');
		return $list;
	}
}
...
if ( !function_exists( 'themerex_post_get_blog_title' ) ) {
	...
	function themerex_post_get_blog_title($title, $page) {
		...
		if ( $page == 'blog' )
			$title = esc_html__( 'All Posts', 'kidsplanet');
		else if ( $page == 'author' ) {
			$curauth = (get_query_var('author_name')) ? get_user_by('slug', get_query_var('author_name')) : get_userdata(get_query_var('author'));
			$title = sprintf(esc_html__('Author page: %s', 'kidsplanet'), $curauth->display_name);
		} else if ( $page == 'error404' )
			$title = esc_html__('URL not found', 'kidsplanet');
		else if ( $page == 'search' )
			$title = sprintf( esc_html__( 'Search: %s', 'kidsplanet'), get_search_query() );
		else if ( $page == 'archives_day' )
			$title = sprintf( esc_html__( 'Daily Archives: %s', 'kidsplanet'), themerex_get_date_translations(get_the_date()) );
		else if ( $page == 'archives_month' )
			$title = sprintf( esc_html__( 'Monthly Archives: %s', 'kidsplanet'), themerex_get_date_translations(get_the_date( 'F Y' )) );
		else if ( $page == 'archives_year' )
			$title = sprintf( esc_html__( 'Yearly Archives: %s', 'kidsplanet'), get_the_date( 'Y' ) );
		 else if ( $page == 'category' )
			$title = sprintf(  '%s', single_cat_title( '', false ) );
		else if ( $page == 'tag' )
			$title = sprintf( esc_html__( 'Tag: %s', 'kidsplanet'), single_tag_title( '', false ) );
		else if ( $page == 'attachment' )
			$title = sprintf( esc_html__( 'Attachment: %s', 'kidsplanet'), themerex_get_post_title());
		else if ( $page == 'single' )
