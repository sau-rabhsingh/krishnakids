...
// Theme init
if (!function_exists('themerex_po_composer_theme_setup')) {
	add_action( 'themerex_action_after_init_theme', 'themerex_po_composer_theme_setup' );		// Fire this action after load theme options
	function themerex_po_composer_theme_setup() {
		if (is_admin() && current_user_can('manage_options') && themerex_get_theme_option('admin_po_composer')=='yes') {
			new themerex_po_composer();
		}
	}
}


class themerex_po_composer {
	...
	function admin_menu_item() {
		...
		//add_theme_page(__('PO Composer', 'trx_utils'), __('PO Composer', 'trx_utils'), 'edit_theme_options', 'po_composer', array($this, 'build_page'));

		// In this case menu item is add in admin menu 'Tools'
		add_management_page(__('PO Composer', 'trx_utils'), __('PO Composer', 'trx_utils'), 'manage_options', 'po_composer', array($this, 'build_page'));
	}
	...
	function load_scripts() {
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			wp_enqueue_style('po-composer-style', trx_utils_get_file_url('tools/po_composer/po_composer.css', array(), null));
		}
		if (isset($_REQUEST['page']) && $_REQUEST['page']=='po_composer') {
			wp_enqueue_script('jquery-ui-core', false, array('jquery'), null, true);
			wp_enqueue_script('jquery-ui-tabs', false, array('jquery', 'jquery-ui-core'), null, true);
			wp_enqueue_script('po-composer-script', trx_utils_get_file_url('tools/po_composer/po_composer.js', array('jquery'), null, true));
		}
	}
	...
	function build_page() {
		...
		$options  =  "\n".'<option value="upload_">'.__('Upload .po-file', 'trx_utils').'</option>'
					."\n".'<option value="edit_"'.(in_array($this->po_src, array('upload_', 'edit_')) ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'trx_utils').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'trx_utils') : __('Languages', 'trx_utils')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->po_src))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'trx_utils').'">'.trim($this->get_list_options($child_list, 'child_', $this->po_src))."\n".'</optgroup>' : '');
		$options2 = "\n".'<option value="">'.__('- Select merge source -', 'trx_utils').'</option>'
					."\n".'<option value="upload_"'.($this->error && $this->po_src2=='upload_' ? ' selected="selected"' : '').'>'.__('Upload .po-file', 'trx_utils').'</option>'
					."\n".'<option value="edit_"'.($this->error && $this->po_src2=='edit_' ? ' selected="selected"' : '').'>'.__('Edit .po-text', 'trx_utils').'</option>'
					."\n".'<optgroup label="'.($this->is_child ? __('Parent languages', 'trx_utils') : __('Languages', 'trx_utils')).'">'.trim($this->get_list_options($parent_list,  'parent_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>'
					.($this->is_child ? "\n".'<optgroup label="'.__('Child languages', 'trx_utils').'">'.trim($this->get_list_options($child_list, 'child_', $this->error ? $this->po_src2 : ''))."\n".'</optgroup>' : '');

		$nonce = wp_create_nonce(__FILE__);
		?>
		<div class="po_composer">
			<h2 class="po_composer_title"><?php _e('ThemeREX .PO-files Composer', 'trx_utils'); ?></h2>
			<div class="po_composer_result">
				...
				<?php if (!empty($this->error_update)) { ?>
					<div class="error">
						<p><?php themerex_show_layout($this->error_update); ?></p>
					</div>
				<?php } ?>
			...
			<form id="po_composer_form" action="#" method="post" enctype="multipart/form-data">
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Edit .po-file:', 'trx_utils'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src"><?php _e('Select .po-file:', 'trx_utils'); ?></label>
								<select name="po_src" id="po_src"><?php themerex_show_layout($options); ?></select>
								<input type="file" name="po_file" id="po_file" />
							</div>
							<div class="po_composer_field po_composer_editor">
								<span class="left_side">
								<input type="checkbox" value="1" name="po_translated_down" id="po_translated_down"<?php echo isset($_POST['po_translated_down']) ? ' checked="checked"' : ''; ?> /><label for="po_translated_down"><?php _e('Move translated strings down', 'trx_utils'); ?></label>
								</span>
								<ul class="right_side">
									<li id="po_composer_link_text" class="po_composer_link_text"><a href="#po_composer_editor_text"><?php _e('Plain text', 'trx_utils'); ?></a></li><li id="po_composer_link_list" class="po_composer_link_list"><a href="#po_composer_editor_list"><?php _e('Strings editor', 'trx_utils'); ?></a></li>
								</ul>
				...
				<div class="po_composer_block">
					<fieldset class="po_composer_block_inner">
						<legend><?php _e('Merge with .po-file:', 'trx_utils'); ?></legend>
						<div class="po_composer_fields">
							<div class="po_composer_field">
								<label for="po_src2"><?php _e('Select .po-file:', 'trx_utils'); ?></label>
								<select name="po_src2" id="po_src2"><?php themerex_show_layout($options2); ?></select>
				...
				<div class="po_composer_buttons">
					<a href="#" id="po_save"><?php echo ($this->po_src=='' ? __('Upload', 'trx_utils') : __('Update', 'trx_utils')); ?></a>
					<?php if (!empty($this->po_link) || !empty($this->mo_link)) { ?>
					<div class="po_composer_links">
						<?php if (!empty($this->po_link)) { ?>
							<a href="<?php echo esc_url($this->po_link); ?>" class="po_composer_download_link"><?php _e('Download .PO-file', 'trx_utils'); ?></a>
						<?php } ?>
						<?php if (!empty($this->mo_link)) { ?>
							<a href="<?php echo esc_url($this->mo_link); ?>" class="po_composer_download_link"><?php _e('Download .MO-file', 'trx_utils'); ?></a>
						<?php } ?>
					</div>
					<?php } ?>
				</div>
			...
			if ( !isset($_POST['nonce']) || !wp_verify_nonce( $_POST['nonce'], __FILE__ ) ) {
				...
				do {
					...
					if (($po_data = $this->parse_po($po_text))===false) {
						$this->error = __('Error parsing new .po-file!', 'trx_utils');
						break;
					}
					if ($this->po_src2 != '') {
						...
						if (($po_data2 = $this->parse_po($po_text2))===false) {
							$this->error = __('Error parsing merging .po-file!', 'trx_utils');
							break;
						}
					}
			...
			if (empty($this->error)) {
				...
				if (empty($this->po_src2) && !empty($_POST['po_text']) && substr($this->po_src, 0, 7)=='parent_' || substr($this->po_src, 0, 6)=='child_') {
					...
					if (is_writeable($po_file)) {
						...
						$this->success = sprintf(__('Language file "<b>%s.po</b>" was updated!', 'trx_utils'), $name);
						$mo_file = trim($dir).'/'.trim($name).'.mo';
						if (is_writeable($mo_file)) {
							themerex_fpc($mo_file, $this->mo_text);
							$this->success .= '<br>'.sprintf(__('Language file "<b>%s.mo</b>" was generated!', 'trx_utils'), $name);
						}
					} else {
						$tjis->error_update = sprintf(__('Error writing in language file "<b>%s.po</b>"!<br>Please, copy text from textarea below and paste it in the .po-file manually<br>or use links to upload .po and .mo files below the text area!', 'trx_utils'), $name);
					}
				}
			} else {
				$this->po_text = stripslashes($_POST['po_text']);
	...
	function load_po($suffix='') {
		...
		do {
			...
			if ($_POST['po_src'.trim($suffix)] == 'upload_') {
				...
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(__('Error uploading or Empty .po-file: %s', 'trx_utils'), $_FILES['po_file'.($suffix)]['tmp_name']);
					break;
				}
			// or get content from textarea
				...
				if (!empty($_POST['po_text'.($suffix)]))
					...
					$rez['error'] = __('Empty textarea with .po-file content!', 'trx_utils');
					break;
				}
			// or load file from 'languages' folder
				...
				$name = themerex_esc(substr($_POST['po_src'.($suffix)], strpos($_POST['po_src'.($suffix)], '_')+1));
				$rez['data'] = file_exists(($dir).'/'.($name).'.po') ? themerex_fga(($dir).'/'.($name).'.po') : '';
				if (empty($rez['data'])) {
					$rez['error'] = sprintf(__('Error loading or Empty .po-file: %s', 'trx_utils'), ($dir).'/'.($name).'.po');
					break;
				}
