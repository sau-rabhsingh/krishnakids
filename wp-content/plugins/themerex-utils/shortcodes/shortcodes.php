		...
		if ($style == 2) {
			...
			$output .= '<div class="sc_columns columns_wrap">'
				...
				. '<div class="sc_contact_form_address_field">'
					. '<span class="sc_contact_form_address_label">'.__('Address', 'trx_utils').'</span>'
					. '<span class="sc_contact_form_address_data">'.trim($address_1).(!empty($address_1) && !empty($address_2) ? ', ' : '').$address_2.'</span>'
				. '</div>'
				. '<div class="sc_contact_form_address_field">'
					. '<span class="sc_contact_form_address_label">'.__('We are open', 'trx_utils').'</span>'
					. '<span class="sc_contact_form_address_data">'.trim($open_hours).'</span>'
				. '</div>'
				. '<div class="sc_contact_form_address_field">'
					. '<span class="sc_contact_form_address_label">'.__('Phone', 'trx_utils').'</span>'
					. '<span class="sc_contact_form_address_data">'.trim($phone).(!empty($phone) && !empty($fax) ? ', ' : '').$fax.'</span>'
				. '</div>'
				. '<div class="sc_contact_form_address_field">'
					. '<span class="sc_contact_form_address_label">'.__('E-mail', 'trx_utils').'</span>'
		...
		$output .= '<form' . ($id ? ' id="'.esc_attr($id).'"' : '') . ' data-formtype="'.($content ? 'custom' : 'contact').'" method="post" action="' . esc_url($action ? $action : $THEMEREX_GLOBALS['ajax_url']) . '">'
					. ($content != '' && themerex_param_is_on($custom)
						...
						: '<div class="sc_contact_form_info">'
								.'<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_username">' . __('Name', 'trx_utils') . '</label><input id="sc_contact_form_username" type="text" name="username" placeholder="' . __('Name *', 'trx_utils') . '"></div>'
								. '<div class="sc_contact_form_item sc_contact_form_field label_over"><label class="required" for="sc_contact_form_email">' . __('E-mail', 'trx_utils') . '</label><input id="sc_contact_form_email" type="text" name="email" placeholder="' . __('E-mail *', 'trx_utils') . '"></div>'
							.'</div>'
							.'<div class="sc_contact_form_item sc_contact_form_message label_over"><label class="required" for="sc_contact_form_message">' . __('Message', 'trx_utils') . '</label><textarea id="sc_contact_form_message" name="message" placeholder="' . __('Message', 'trx_utils') . '"></textarea></div>'
                        ...
                        .'<div class="sc_contact_form_item sc_contact_form_button"><button class="sc_button '. esc_attr($button_size).'" ' . (!empty($privacy) ? ' disabled="disabled"' : '') . '>'. esc_attr($button_text).'</button></div>'
						)
...
if (!function_exists('themerex_sc_emailer')) {
	function themerex_sc_emailer($atts, $content = null) {
		...
		$output = '<div' . ($id ? ' id="'.esc_attr($id).'"' : '')
				...
				. '<input type="text" class="sc_emailer_input" name="email" value="" placeholder="'.__('Please, enter you email address.', 'trx_utils').'">'
				. '<button class="sc_emailer_button icon-mail" title="'.__('Submit', 'trx_utils').'" data-group="'.($group ? $group : __('E-mailer subscription', 'trx_utils')). (!empty($privacy) ? ' disabled="disabled"' : '') . '"></button>'
                . '</div>'
            . ((!empty($privacy)) ? '<div class="sc_form_field sc_form_field_checkbox">'
                  . '<input type="checkbox" id="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '" name="i_agree_privacy_policy" class="sc_form_privacy_checkbox" value="1">
                  <label for="i_agree_privacy_policy_sc_form_' . esc_attr($cnt) . '">' . $privacy . '</label></div>' : '')
...
if (!function_exists('themerex_sc_googlemap_marker')) {
	function themerex_sc_googlemap_marker($atts, $content = null) {
		...
		$content = empty($content) ? empty($atts['description']) ? '' : $atts['description'] : $content;
		global $THEMEREX_GLOBALS;
		$THEMEREX_GLOBALS['sc_googlemap_markers'][] = array(
			'id' => $id,
			'title' => $title,
					...
					. '>
						<div class="search_form_wrap">
							<form role="search" method="get" class="search_form" action="' . esc_url( home_url( '/' ) ) . '">
								<button type="submit" class="search_submit icon-search" title="' . ($state=='closed' ? __('Open search', 'trx_utils') : __('Start search', 'trx_utils')) . '"></button>
...
if (!function_exists('themerex_sc_section')) {
	function themerex_sc_section($atts, $content=null){	
		...
		$css .= themerex_get_css_position_from_values('!'.($top), '!'.($right), '!'.($bottom), '!'.($left))
			.($color !== '' ? 'color:' . esc_attr($color) . ';' : '')
			.($bg_color !== '' && (int)$bg_overlay==0 ? 'background-color:' . esc_attr($bg_color) . ';' : '')
			.($bg_image !== '' ? 'background-image:url(' . esc_url($bg_image) . ');' : '')
