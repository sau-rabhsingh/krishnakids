...
if ( !function_exists( 'themerex_template_services_4_theme_setup' ) ) {
	add_action( 'themerex_action_before_init_theme', 'themerex_template_services_4_theme_setup', 1 );
	function themerex_template_services_4_theme_setup() {
		themerex_add_template(array(
			'layout' => 'services-4',
			'template' => 'services-4',
			'mode'   => 'services',
			'need_columns' => true,
			'title'  => esc_html__('Services /Style 4/', 'kidsplanet')
		));
	}
}

// Template output
if ( !function_exists( 'themerex_template_services_4_output' ) ) {
	function themerex_template_services_4_output($post_options, $post_data) {
		$show_title = true;
		...
		?>
			<div<?php themerex_show_layout($post_options['tag_id'] ? ' id="'.esc_attr($post_options['tag_id']).'"' : ''); ?>
				...
				?><span class="sc_services_item_title"><?php themerex_show_layout($post_data['post_title']); ?></span><?php
				if ((!isset($post_options['links']) || $post_options['links']) && !empty($post_data['post_link'])) {
					?></a><?php
				}
				?>
